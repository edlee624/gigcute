...
const seeker = {
  ...
  async saveFull({ profile = {}, workHistory = [], education = [], promptAnswers = [] }) {
    ...
    if (error && /schema cache|could not find .* column|column .* does not exist/i.test(error.message || '')) {
      const { resume_url, portfolio, certifications, desired_titles, desired_salary_min, desired_salary_max, personality_type, personality_url, strengths, strengths_url, ...rest } = profile;
