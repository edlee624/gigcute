-- ============================================================================
-- GigCute — CliftonStrengths on the seeker profile (alongside Myers-Briggs).
--   strengths     : the seeker's Top 5 themes, IN RANK ORDER (index 0 = #1).
--                   Values are theme names, e.g. {'Consistency','Analytical',...}.
--                   Domains (Executing / Influencing / Relationship Building /
--                   Strategic Thinking) are derived in the UI, not stored.
--   strengths_url : their shareable results link (optional), same idea as
--                   personality_url. Taken free at testmypersona.com.
-- public_profile() recreated to expose both.
-- ============================================================================
alter table public.seeker_profiles
  add column if not exists strengths     text[] not null default '{}',
  add column if not exists strengths_url text;
...
returns jsonb language sql stable security definer set search_path = public as $$
  select case
    when not exists (
      ...
      'personality_type', sp.personality_type,
      'personality_url',  sp.personality_url,
      'strengths',        coalesce(to_jsonb(sp.strengths), '[]'::jsonb),
      'strengths_url',    sp.strengths_url,
