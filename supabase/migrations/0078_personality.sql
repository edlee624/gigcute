-- ============================================================================
-- GigCute — personality (Myers-Briggs) on the seeker profile.
--   personality_type : 4-letter MBTI code, e.g. 'ISFP' (optionally with -A/-T).
--   personality_url  : the seeker's own shareable results link (optional), so
--                      viewers can open the full write-up. Taken for free at
--                      testmypersona.com (guidance lives in the UI).
-- public_profile() recreated to expose both.
-- ============================================================================
alter table public.seeker_profiles
  add column if not exists personality_type text,
  add column if not exists personality_url  text;
