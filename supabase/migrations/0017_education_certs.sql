-- ============================================================================
-- GigCute — Education in the public profile + Certifications
-- Education already has a table (migration 0001) but was never surfaced by the
-- public_profile RPC. Certifications are new and, like portfolio, are stored as
-- a JSON array on the seeker_profiles row (display-only).
-- Certification item shape: { "name": text, "issuer": text, "year": text, "url": text }
-- ============================================================================
alter table public.seeker_profiles add column if not exists certifications jsonb not null default '[]'::jsonb;
